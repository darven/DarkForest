...
class Plugin {
    constructor() {
        ...
        this.minPlanetLevelTo = PlanetLevel.THREE;
    ...
    render(container) {
		...
		let optAll = document.createElement('option');
            optAll.value = `-1`;
            optAll.innerText = `All,No Quasar`;
        ...
        planetType.onchange = (evt) => {
            try {
				...
				message.innerText = `Selected PlanetType=${this.planetType} ${getPlanetName(this.planetType)}`;
            } catch (e) {
        ...
        button.onclick = () => {
            ...
            if (planet) {
                ...
                message.innerText = `Crawling ${moves} ${getPlanetName(this.planetType)}.`;
            } else {
...
function getPlanetName(pType){
	if(pType < 0)
		return 'Stars';
	else
		return `${PlanetTypeNames[pType]}`;
}
